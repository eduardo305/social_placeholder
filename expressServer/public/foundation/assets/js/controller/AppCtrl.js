...
app.controller('AppCtrl', function($scope, $rootScope, $mdSidenav, User, Friendship, $http, $mdDialog, $mdToast, $cookies, jwtHelper, $filter, $state, md5) {
    ...
    $scope.acceptFriend = function(tile) {
        var id = tile.userRequester._id;
        $scope.tiles = $filter('filter')($scope.tiles, {
            _id: '!' + id
        });
        Friendship.accept({
            ...
            $rootScope.$broadcast('friendship:requested');
            $mdToast.show({
                template: '<div data-alert class="alert-box success fixed">Friendship accepted!</div>',
                hideDelay: 3000
            });
        })
    };

    $scope.rejectFriend = function(tile) {
        var id = tile.userRequester._id;
        $scope.tiles = $filter('filter')($scope.tiles, {
            _id: '!' + id
        });
